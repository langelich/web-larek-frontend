...
4. Презентер обрабатывает событие `catalog:selected` и отправляет запрос на сервер для получения данных выбранной карточки
5. Получив данные карточки, презентер вызывает метод `setData(cardData: IProductItem & TProductBase): void` класса `Card`
6. Класс отображения `Modal` принимает в метод `render(content: IContent): HTMLElement` полученную карточку с данными выбранного товара и отображает о модальном окне подробную информацию.
...
Методы:
- `get contentElement(): HTMLElement` - геттер для получения корзины;
...
- `removeBasket(): void` - очищает корзину после заказа
...
- `getValues(): string` - получает значение инпута в виде строки;
...
Поля:
- `_quantity: HTMLElement` - порядковые номера для карточек в корзине;

Метод:
`set quantity(value: number): number` - устанавливает порядковые номера;
...
- `catalog:added` - получены карточек на страницу;
- `basket:add` - добавление товаров в массив
- `basket:delete` - удаление товаров из массива
